...
BLOCKLIST_1HOSTS="$DNSCRYPT_CONF_DIR/blocked-1hosts.txt"
BLOCKLIST_MERGED="$DNSCRYPT_CONF_DIR/blocked-full-merger.txt"
UPDATE_SCRIPT="$DNSCRYPT_CONF_DIR/update-blocklists.sh"
...
if [[ "$1" == "-ads-only" ]]; then
    BLOCKLIST_MODE="ads"
    echo -e "${YEL}Режим: только реклама (-ads-only)${NC}"
elif [[ "$1" == "-nsfw-only" ]]; then
    BLOCKLIST_MODE="nsfw"
    echo -e "${YEL}Режим: только nsfw (-nsfw-only)${NC}"
else
    echo -e "${YEL}Режим: nsfw + реклама (по умолчанию)${NC}"
fi

# ── 6. Скачивание списков блокировок ──────────────────────────────────────────
# ВАЖНО: скачиваем ДО остановки сервиса, чтобы не ломать DNS
echo -e "${YEL}Скачивание списков блокировок oisd...${NC}"

if [[ "$BLOCKLIST_MODE" == "both" || "$BLOCKLIST_MODE" == "nsfw" ]]; then
...
if [[ "$BLOCKLIST_MODE" == "both" || "$BLOCKLIST_MODE" == "ads" ]]; then
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GRN}✅ blocked-big-ads.txt: $(wc -l < "$BLOCKLIST_SMALL") строк${NC}"
    else
        echo -e "${RED}❌ Ошибка скачивания big.oisd.nl${NC}"
        exit 1
    fi
    curl -fsSL "https://raw.githubusercontent.com/badmojr/1Hosts/master/Lite/wildcards.txt" -o "$BLOCKLIST_1HOSTS"
    if [ $? -eq 0 ]; then
        echo -e "${GRN}✅ blocked-1hosts.txt: $(wc -l < "$BLOCKLIST_1HOSTS") строк${NC}"
    else
        echo -e "${RED}❌ Ошибка скачивания 1Hosts Lite${NC}"
        exit 1
    fi
fi

if [[ "$BLOCKLIST_MODE" == "both" ]]; then
    cat "$BLOCKLIST_NSFW" "$BLOCKLIST_SMALL" "$BLOCKLIST_1HOSTS" | grep -v '^# ' | grep -v '^$' | awk '!seen[$0]++' > "$BLOCKLIST_MERGED"
elif [[ "$BLOCKLIST_MODE" == "nsfw" ]]; then
    grep -v '^# ' "$BLOCKLIST_NSFW" | grep -v '^$' > "$BLOCKLIST_MERGED"
elif [[ "$BLOCKLIST_MODE" == "ads" ]]; then
    cat "$BLOCKLIST_SMALL" "$BLOCKLIST_1HOSTS" | grep -v '^# ' | grep -v '^$' | awk '!seen[$0]++' > "$BLOCKLIST_MERGED"
fi
echo -e "${GRN}✅ blocked-full-merger.txt: $(wc -l < "$BLOCKLIST_MERGED") строк${NC}"
...
# ── 10. Скрипт обновления списков блокировок ──────────────────────────────────
if [[ "$BLOCKLIST_MODE" == "both" ]]; then
    cat > "$UPDATE_SCRIPT" << 'UPDEOF'
#!/bin/bash
DNSCRYPT_CONF_DIR="/etc/dnscrypt-proxy"
BLOCKLIST_NSFW="$DNSCRYPT_CONF_DIR/blocked-nsfw.txt"
BLOCKLIST_SMALL="$DNSCRYPT_CONF_DIR/blocked-big-ads.txt"
...
# ── 11. Cron — запуск скрипта обновления в 6:00 ──────────────────────────────
cat > "/etc/cron.d/dnscrypt-blocklists" << EOF
0 6 * * * root $UPDATE_SCRIPT
EOF
chmod 644 /etc/cron.d/dnscrypt-blocklists
echo -e "${GRN}✅ Cron обновление списков: ежедневно в 6:00 (режим: $BLOCKLIST_MODE)${NC}"
...
if [[ "$BLOCKLIST_MODE" == "both" || "$BLOCKLIST_MODE" == "nsfw" ]]; then
    ...
    if [ "$STATUS" = "NXDOMAIN" ] || [ -z "$BLOCKED" ]; then
        ...
        NSFW_CHECK_INFO="${YEL}NSFW блокировка:${NC}         ${GRN}BLOCKED ($STATUS)${NC}"
    else
        echo -e "${YEL}WARN — ответ: $BLOCKED${NC}"
        NSFW_CHECK_INFO="${YEL}NSFW блокировка:${NC}         ${RED}WARN ($BLOCKED)${NC}"
    fi
fi
...
[[ "$BLOCKLIST_MODE" == "both" || "$BLOCKLIST_MODE" == "ads" ]] && BLOCKLIST_INFO+="${YEL}blocked-big-ads.txt:${NC}     ${CYN}$(wc -l < $BLOCKLIST_SMALL) строк${NC}\n"
[[ "$BLOCKLIST_MODE" == "both" || "$BLOCKLIST_MODE" == "ads" ]] && BLOCKLIST_INFO+="${YEL}blocked-1hosts.txt:${NC}      ${CYN}$(wc -l < $BLOCKLIST_1HOSTS) строк${NC}\n"
BLOCKLIST_INFO+="${YEL}blocked-full-merger.txt:${NC} ${CYN}$(wc -l < $BLOCKLIST_MERGED) строк (режим: $BLOCKLIST_MODE)${NC}"
...
$(echo -e "$BLOCKLIST_INFO")
$([ -n "$NSFW_CHECK_INFO" ] && echo -e "$NSFW_CHECK_INFO")
${YEL}Cron:${NC}                    ${GRN}/etc/cron.d/dnscrypt-blocklists${NC}
${YEL}Update script:${NC}           ${GRN}$UPDATE_SCRIPT${NC}
